...
`LIB_VERSION` in `bugbottle.php` is what the enqueued URL is versioned by, so
bumping it is what makes browsers fetch the new file. The plugin's own
`VERSION` tracks the plugin. See
[Following a library release](#following-a-library-release) for the command
that moves both bundles to a new version.
...
### Following a library release

Keeping up used to be an hour of careful, boring work: copy one bundle, rebuild
the other against the same version, bump `LIB_VERSION` in two files, put both
md5s in the changelog, fix the line in `readme.txt` that names the version, and
then read the library's changelog to work out what any of it means here. The
first six of those are a script:

```bash
bin/sync-library.sh v0.15.0
```

It reads `dist/bugbottle.js` from the library at that tag — `../bugbottle` when
that is a checkout, the public repository otherwise, `--source` to say
elsewhere — copies it over `assets/bugbottle.js`, writes the version into
`bugbottle.php` and into `bin/build-screenshot-bundle.sh`, rebuilds
`assets/bugbottle-screenshot.js` against `bugbottle@<version>` so the two
bundles cannot come from different releases, and writes both md5s into a
`CHANGELOG.md` stub under **Unreleased**, the line in `readme.txt` and the
version this section names.

It refuses to run on a dirty tree, and it is idempotent: the changelog stub
lives between two markers and is replaced rather than appended, so running it
twice leaves the same tree. `--no-bundle` skips the rebuild, which is the only
step needing node, npm and the registry — for looking before leaping, never for
a release.

Then it prints every `## x.y.z` section of the library's own changelog strictly
after the version that was bundled and up to the new one, followed by a
checklist. That part is deliberately not automated. Whether a library release
needs a new setting, a validator, a meta key, a Markdown section or nothing at
all is a judgement about this plugin, and the script has no business making it.

What it does not touch: the expectations in `tests/test-report-parity.php`,
which are the library's own output and have to be regenerated by hand when
`report-core.ts` or `markdown.ts` move; the plugin's own `VERSION` and
`Stable tag`, because following the library is not the same event as releasing
the plugin; and the prose, in either changelog.
