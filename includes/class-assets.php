	...
	private static function config_script(): string {
		...
		$template = implode(
			...
			array(
				...
				'		// The queue POSTs a finished body with a fetch of its own, so on a',
				'		// site with signing keys a report that waited out an outage would',
				'		// otherwise arrive without X-Bugbottle-Signature and be refused with',
				'		// 401 — exactly the report the queue exists to save. Since bugbottle',
				'		// 1.0.0 the queue takes the signer directly and signs the bytes at',
				'		// delivery, once per attempt, which is also what gives the signature',
				'		// a timestamp inside the skew window. Signing at enqueue would be',
				'		// stale by the time it arrives. The plugin used to do this with a',
				'		// `fetch` wrapper of its own; the library does it now.',
				'		if (signer) queueOptions.sign = signer;',
